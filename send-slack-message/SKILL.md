...
- Prefer Slack channel IDs such as `C01LA1HJV8F` over channel names.
...
Use a channel ID, user ID, or conversation ID when available. If the user gives a channel name and Slack returns `channel_not_found`, ask for the channel ID or ask them to invite the bot to the channel.
...
- `channel_not_found`: the channel name is wrong, the bot is not in the private channel, the bot lacks access, or the token belongs to a different workspace. Ask for a channel ID or bot invite.
